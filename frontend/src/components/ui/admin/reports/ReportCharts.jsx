...
function LegendItem({ color, label }) {
  return <span className="inline-flex items-center gap-2 text-xs font-bold text-slate-500"><i className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: color }} />{label}</span>;
}
...
export default function ReportCharts({ data = [], loading = false }) {
  ...
  return (
    <div className="grid min-w-0 gap-5 xl:grid-cols-2">
      <ChartCard icon={CircleDollarSign} eyebrow="Dòng tiền" title="Xu hướng doanh thu" description="Chỉ tính giao dịch đã thanh toán thành công" accent="bg-emerald-200/50" legends={<><LegendItem color="#10b981" label="Doanh thu" /><LegendItem color="#0f766e" label="Đơn đã trả" /></>}>
        <ResponsiveContainer width="100%" height="100%"><AreaChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}><defs><linearGradient id="reportRevenue" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#10b981" stopOpacity={0.38} /><stop offset="100%" stopColor="#10b981" stopOpacity={0.02} /></linearGradient></defs><CartesianGrid stroke="#e2e8f0" strokeDasharray="4 6" vertical={false} /><XAxis dataKey="label" axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: "#64748b", fontWeight: 700 }} minTickGap={20} /><YAxis yAxisId="money" axisLine={false} tickLine={false} width={58} tick={{ fontSize: 11, fill: "#94a3b8" }} tickFormatter={compact} /><YAxis yAxisId="orders" orientation="right" hide allowDecimals={false} /><Tooltip content={<ChartTooltip />} cursor={{ stroke: "#94a3b8", strokeDasharray: "4 4" }} /><Area yAxisId="money" type="monotone" dataKey="revenue" name="Doanh thu" stroke="#10b981" strokeWidth={3} fill="url(#reportRevenue)" activeDot={{ r: 5, strokeWidth: 3, stroke: "white" }} /><Line yAxisId="orders" type="monotone" dataKey="paid_orders" name="Đơn đã trả" stroke="#0f766e" strokeWidth={2} dot={false} /></AreaChart></ResponsiveContainer>
      </ChartCard>

      <ChartCard icon={PackageCheck} eyebrow="Vận hành" title="Đơn hàng theo thời gian" description="Phân biệt đơn cha của khách và đơn con theo gian hàng" accent="bg-sky-200/50" legends={<><LegendItem color="#2563eb" label="Đơn cha" /><LegendItem color="#8b5cf6" label="Đơn con" /><LegendItem color="#10b981" label="Hoàn thành" /><LegendItem color="#f43f5e" label="Đã hủy" /></>}>
        <ResponsiveContainer width="100%" height="100%"><ComposedChart data={data} margin={{ top: 8, right: 8, left: -18, bottom: 0 }}><CartesianGrid stroke="#e2e8f0" strokeDasharray="4 6" vertical={false} /><XAxis dataKey="label" axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: "#64748b", fontWeight: 700 }} minTickGap={20} /><YAxis allowDecimals={false} axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: "#94a3b8" }} /><Tooltip content={<ChartTooltip />} cursor={{ fill: "#f1f5f9", opacity: 0.7 }} /><Bar dataKey="orders" name="Đơn cha" fill="#2563eb" radius={[6, 6, 0, 0]} maxBarSize={24} /><Bar dataKey="sub_orders" name="Đơn con" fill="#8b5cf6" radius={[6, 6, 0, 0]} maxBarSize={24} /><Line type="monotone" dataKey="completed_orders" name="Hoàn thành" stroke="#10b981" strokeWidth={3} dot={false} /><Line type="monotone" dataKey="cancelled_orders" name="Đã hủy" stroke="#f43f5e" strokeWidth={2} dot={false} /></ComposedChart></ResponsiveContainer>
      </ChartCard>

      <section className="xl:col-span-2 overflow-hidden rounded-3xl bg-slate-950 p-5 text-white shadow-xl sm:p-6"><div className="flex flex-wrap items-center justify-between gap-4"><div className="flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-white/10"><Users size={20} /></span><div><p className="text-[11px] font-black uppercase tracking-[0.18em] text-indigo-300">Tăng trưởng</p><h2 className="text-lg font-black">Người dùng mới theo từng kỳ</h2></div></div><LegendItem color="#a78bfa" label="Tài khoản mới" /></div><div className="mt-5 h-52"><ResponsiveContainer width="100%" height="100%"><AreaChart data={data}><defs><linearGradient id="reportUsers" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#a78bfa" stopOpacity={0.5} /><stop offset="100%" stopColor="#a78bfa" stopOpacity={0} /></linearGradient></defs><CartesianGrid stroke="#334155" strokeDasharray="4 6" vertical={false} /><XAxis dataKey="label" axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: "#94a3b8" }} minTickGap={20} /><YAxis allowDecimals={false} axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: "#64748b" }} /><Tooltip content={<ChartTooltip />} /><Area type="monotone" dataKey="new_users" name="Người dùng mới" stroke="#a78bfa" strokeWidth={3} fill="url(#reportUsers)" /></AreaChart></ResponsiveContainer></div></section>
    </div>
  );
}
