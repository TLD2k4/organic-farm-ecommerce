...
function SellerHarvestLots() {
  ...
  const pageStats = useMemo(() => {
    return {
      total: meta.total ?? 0,
      active: lots.filter((lot) => Number(lot.display_status || lot.status) === 1).length,
      warning: lots.filter((lot) => isExpiringSoon(lot.expiry_date)).length,
      out: lots.filter((lot) => Number(lot.display_status || lot.status) === 3).length,
    };
  }, [lots, meta.total]);
  ...
  const openEditModal = async (lot) => {
    ...
    try {
      ...
      setForm({
        ...
        status: [1, 2].includes(Number(detail.status)) ? String(detail.status) : "",
        note: detail.note || "",
      });
    } catch (err) {
      setForm({
  ...
  const handleDelete = async (lot) => {
    const ok = window.confirm(
      `Bạn có chắc muốn xóa lô "${lot.lot_code}" không?`
    );
  ...
  const handleToggleStatus = async (lot) => {
    ...
    try {
      ...
      toast.success(
        nextStatus === 1
          ? "Đã bật bán lô sản phẩm."
          : "Đã tạm ẩn lô sản phẩm."
      );
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
        <div className="mb-5 grid grid-cols-[1fr_1fr_0.8fr_auto_auto] gap-3">
          ...
          >
            <option value="">
              {optionLoading ? "Đang tải sản phẩm..." : "Tất cả sản phẩm"}
            </option>
          ...
          >
            ...
            <option value="4">Hết hạn sử dụng</option>
          </select>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-3 py-3">
                        ...
                        <button
                        title={canEdit ? "Sửa lô" : "Lô đã hết hạn, không thể sửa"}
...
function HarvestLotFormModal({
  ...
  const cannotEditQuantity = !isCreate && quantitySold > 0;

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-5">
      <div className="max-h-[94vh] w-[74vw] max-w-[1080px] overflow-y-auto rounded-2xl bg-white shadow-xl">
          ...
          <div className="rounded-2xl border border-slate-100 p-5">
            ...
            {cannotEditQuantity && (
              <div className="mt-4 rounded-xl bg-orange-50 px-4 py-3 text-sm font-bold text-orange-700">
                Lô đã phát sinh bán hàng nên không được sửa số lượng nhập.
              </div>
            )}
          </div>
...
function HarvestLotDetailModal({ lot, loading, getImageUrl, onClose }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-5">
      <div className="max-h-[94vh] w-[72vw] max-w-[1080px] overflow-y-auto rounded-2xl bg-white shadow-xl">
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-100 bg-white px-6 py-4">
          <div>
            <h2 className="text-xl font-extrabold text-slate-950">
                ...
                <div className="mb-4 flex items-start justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-sm font-semibold text-slate-400">
                      {lot.product?.name} · {lot.category?.name || "Chưa có danh mục"}
                    </p>
                  </div>
              ...
              {lot.certificate ? (
                <div className="grid grid-cols-4 gap-3">
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="col-span-4 inline-flex w-fit items-center gap-2 rounded-xl bg-green-50 px-4 py-3 text-sm font-extrabold text-green-700"
                  >
                    <FileText size={17} />
                    Xem file chứng chỉ
                  </a>
  ...
  return (
    <label className="block">
      ...
      <input
        ...
        className="h-11 w-full rounded-xl border border-slate-200 px-3 text-sm font-semibold outline-none focus:border-green-500 disabled:bg-slate-50 disabled:text-slate-400"
      />
    </label>
  );
}
